...
unless defined?(Abaqus::Element::C3D20)
...
module Abaqus
  class Element
    class C3D20 < Element
      def initialize(i,n1,n2,n3,n4,n5,n6,n7,n8,n9,n10,n11,n12,n13,n14,n15,n16,n17,n18,n19,n20)
      ...
      def [](n)
        if n < 20 && n > -21
          ...
          raise ArgumentError,"index must be between 0 to 7"
        end
      end
      def type
        "C3D20"
...
if $0 == __FILE__
  raise NotImplimentedError, "Not yet modified."
  require 'test/unit'
  require 'flexmock/test_unit'
  class TestElementC3D20 < Test::Unit::TestCase

    def setup
      @c3d8id = 1
      @c3d8nodes = [1,2,3,6,9,8,7,4,]
      @c3d8rid = 2
      @c3d8rnodes = [11,12,13,16,19,18,17,14]
    end
    def c3d8
      Abaqus::Element::C3D8.new(@c3d8id,*@c3d8nodes)
    end
    def c3d8r
      Abaqus::Element.new("C3D8R",@c3d8rid,*@c3d8rnodes)
    end
    def teardown
      Abaqus::Element.clear
    end

    def test_C3D8_new_keeps_order_of_node
      e = c3d8

      8.times do |i|
        assert_equal(@c3d8nodes[i],e[i])
      end
    end

    def test_C3D8_must_raise_ArgumentError_if_node_id_query_with_over_3
      e = c3d8
      assert_raise(ArgumentError){
        e[8]
      }
    end
    def test_C3D8_must_raise_ArgumentError_if_node_id_query_with_less_than_minus_5
      e = c3d8
      assert_raise(ArgumentError){
        e[-9]
      }
    end
    def test_C3D8_nid_minus_8_must_return_same_value_of_idx_0
      e = c3d8
      assert_equal(e[0],e[-8])
    end
    def test_C3D8_type_must_return_C3D8
      assert_equal("C3D8",c3d8.type)
    end
    def test_C3D8_new_added_to_ElementList
      e = c3d8
      assert_equal(e, Abaqus::Element[@c3d8id])
    end

    def test_C3D8_new_increase_Element_size
      assert_equal(0, Abaqus::Element.size)
      c3d8
      assert_equal(1, Abaqus::Element.size)
    end

    def test_C3D8_i_must_return_created_id
      assert_equal(@c3d8id, c3d8.i)
    end

    def test_C3D8_nodes_method_returns_array_of_nodes_with_its_order
      assert_equal(@c3d8nodes,c3d8.nodes)
    end

    def test_modificaion_of_retval_of_C3D8_nodes_should_not_affect_original
      e = c3d8
      a = c3d8.nodes
      a[0] = 8
      c3d8.nodes[1] = 9
    ...
    def test_stored_element_must_be_original_type_C3D8
      e = c3d8
      assert_equal(e, Abaqus::Element[@c3d8id])
    end
    ...
    def test_creation_of_first_element_change_nextid_for_the_elements_id_plus_1
      eid = rand(7438)
      e = Abaqus::Element::C3D8.new(eid, * @c3d8nodes )
      assert_equal(eid+1,Abaqus::Element.nextid)
    end
  end
  class TestC3D8Parse < Test::Unit::TestCase
    def setup
      @cmd = "*Element, type=C3D8"
...
3, 7, 8, 9,12,15,14,13,10
    ...
    def test_created_element_type_must_be_C3D8_for_one_elements
      try_parse(@cmd,@str1)
      e = Abaqus::Element[@ids[0]]
      assert_equal("C3D8",e.type)
    end
    def test_created_element_has_same_element_of_given
      try_parse(@cmd,@str1)
      assert_equal([1,2,3,6,9,8,7,4],Abaqus::Element[1].nodes)
    end
    def test_parsed_result_of_C3D8RH_will_be_subclass_of_C3D8
      try_parse("*ELEMENT,type=C3D8RH",@str1)
      assert_kind_of(Abaqus::Element::C3D8,Abaqus::Element[1])
    end
    def test_parsed_result_of_C3D8RH_must_have_type_of_C3D8RH
      try_parse("*ELEMENT,type=C3D8RH",@str1)
      assert_equal("C3D8RH", Abaqus::Element[1].type)
    end
    def test_multiple_definition_of_element_can_be_parsed_correctly
      cmd = "*Element,Elset=A,type=C3D8"
      second_cmd = "*Element,elset=B,type=C3D8R"
      ans = [
        "1,  1, 2, 3, 6, 9, 8, 7, 4",
      ...
      assert_equal("C3D8", Abaqus::Element[1].type)
      assert_equal(ns[1], Abaqus::Element[2].nodes)
      assert_equal("C3D8", Abaqus::Element[2].type)
      ...
      assert_equal("C3D8R", Abaqus::Element[3].type)
      assert_equal(ns[3], Abaqus::Element[4].nodes)
      assert_equal("C3D8R", Abaqus::Element[4].type)
    end
  ...
  class TestElementC3D8ParseForNonSequencialEID < Test::Unit::TestCase
    def setup
      cmd = "*ELEment, type=C3D8, elset=A"
      body = flexmock("mIO")
  ...
  class TestElsetInC3D8 < Test::Unit::TestCase
    def setup
      @elset_name = "TestElements"
      @cmd = "*element, type=C3D8, elset=#{@elset_name}"
      @str1 = <<-NNN
1, 1,2,3,6,9,8,7,4
      NNN
      @str2 = <<-KKK
